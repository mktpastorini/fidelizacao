...
-- 2. Trigger
-- Remove o trigger antigo se existir, para garantir que estamos usando a versão mais recente
DROP TRIGGER IF EXISTS on_delivery_item_ready ON public.itens_pedido;

CREATE TRIGGER on_delivery_item_ready
AFTER UPDATE ON public.itens_pedido
FOR EACH ROW
EXECUTE FUNCTION public.handle_delivery_item_update();
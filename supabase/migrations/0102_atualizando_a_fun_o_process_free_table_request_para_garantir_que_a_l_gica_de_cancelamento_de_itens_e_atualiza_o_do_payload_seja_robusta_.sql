...
BEGIN
    ...
    SELECT id, cliente_id, acompanhantes INTO v_open_order_id, v_cliente_principal_id, v_occupants_json
    FROM public.pedidos
    ...
    SET 
        ...
        approved_at = NOW(),
        payload = jsonb_set(
            jsonb_set(payload, '{cancelled_items}', COALESCE(v_cancelled_items, '[]'::jsonb)),
            '{occupants_at_cancellation}', COALESCE(v_occupants_json, '[]'::jsonb)
        )

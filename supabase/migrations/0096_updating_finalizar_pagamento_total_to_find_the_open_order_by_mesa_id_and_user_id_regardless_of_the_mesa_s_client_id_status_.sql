...
BEGIN
    -- 1. Obter informações essenciais do pedido (Busca pelo ID do pedido e mesa, garantindo que esteja aberto e pertença ao usuário)
    ...
    -- Se o cliente principal (v_cliente_id) for NULL neste ponto (o que não deveria acontecer se o pedido foi criado corretamente, mas como fallback)
    -- Usamos o cliente_id da MESA como fallback, se existir.
    IF v_cliente_id IS NULL THEN
        SELECT cliente_id INTO v_cliente_id FROM public.mesas WHERE id = p_mesa_id;
    END IF;

    -- Se ainda assim não houver cliente principal, não podemos atribuir os itens da mesa.
    IF v_cliente_id IS NULL THEN
        RAISE EXCEPTION 'Não foi possível determinar o cliente principal para atribuir os itens da mesa.';
    END IF;

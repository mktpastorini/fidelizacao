...
async function getComprefaceSettings(supabaseAdmin: any) {
  console.log("[add-cook-face-examples] Buscando configurações do CompreFace do Superadmin principal...");
  ...
  if (profileError || !superadminProfile) {
    console.error("[add-cook-face-examples] Erro ao buscar Superadmin:", profileError);
    return { settings: null, error: new Error("Falha ao encontrar o Superadmin principal.") };
  }
  
  const superadminId = superadminProfile.id;
  console.log(`[add-cook-face-examples] Superadmin ID encontrado: ${superadminId}`);
  ...
  if (settingsError) {
    console.error(`[add-cook-face-examples] Erro ao buscar configurações do Superadmin ${superadminId}:`, settingsError);
...
serve(async (req) => {
  console.log("--- [add-cook-face-examples] INICIANDO EXECUÇÃO ---");
  
  if (req.method === 'OPTIONS') {
    console.log("[add-cook-face-examples] Requisição OPTIONS recebida.");
  ...
  try {
    console.log("[add-cook-face-examples] 1/7: Parsing body da requisição...");
    const { subject, image_urls } = await req.json();
    console.log(`[add-cook-face-examples] 1/7: Body recebido - subject: ${subject}, image_urls.length: ${image_urls?.length}`);
    
    if (!subject || typeof subject !== 'string' || subject.trim() === '') {
      throw new Error(`Parâmetro 'subject' (ID do cozinheiro) inválido ou ausente.`);
    }
    if (!image_urls || !Array.isArray(image_urls) || image_urls.length === 0) {
      throw new Error("`image_urls` deve ser um array com pelo menos uma URL.");
    }

    console.log("[add-cook-face-examples] 2/7: Autenticando usuário logado...");
    ...
    console.log(`[add-cook-face-examples] 2/7: Usuário autenticado: ${user.id}`);

    // 3. Buscando configurações do CompreFace do Superadmin
    console.log("[add-cook-face-examples] 3/7: Buscando configurações globais do CompreFace...");
    ...
    console.log(`[add-cook-face-examples] 3/7: Configurações carregadas.`);

    let successCount = 0;
    const errors = [];

    console.log(`[add-cook-face-examples] 4/7: Iniciando loop para ${image_urls.length} imagens...`);
    for (const [index, imageUrl] of image_urls.entries()) {
      const logPrefix = `[add-cook-face-examples] Imagem ${index + 1}/${image_urls.length}:`;
      ...
      try {
        ...
        // O subject é o ID do cozinheiro.
    ...
    console.log(`[add-cook-face-examples] 7/7: Loop concluído. Sucessos: ${successCount}, Erros: ${errors.length}`);
    if (successCount === 0) {
    ...
    console.error("--- [add-cook-face-examples] ERRO FATAL ---");

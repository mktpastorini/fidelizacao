...
/* ---------- defaults ---------- */
const defaultProps: Required<SplashCursorProps> = {
  SIM_RESOLUTION: 128,
  DYE_RESOLUTION: 1440,
...
/* ---------- helper functions (outside useEffect for clarity) ---------- */
...
export function SplashCursor(userProps: SplashCursorProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const frameIdRef = useRef<number>();

  const config = useMemo(() => ({ ...defaultProps, ...userProps }), [userProps]);
  ...
  useEffect(() => {
    ...
    try {
      ({ gl, ext, isWebGL2 } = getWebGLContext(canvas));
    } catch (e) {
      console.error(e);
      return;
    }
    ...
    /* ---------- WebGL context helpers (defined inside useEffect to capture gl) ---------- */

    function getWebGLContext(canvasEl: HTMLCanvasElement): WebGLContext {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,

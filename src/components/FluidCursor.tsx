import React, { useEffect, useRef, withDefaults } from 'react';
...
// Default values (matching the provided Vue defaults)
const defaultProps: Required<FluidCursorProps> = {
  SIM_RESOLUTION: 128,
  DYE_RESOLUTION: 1440,
...
export function FluidCursor(props: FluidCursorProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const finalConfig = { ...defaultProps, ...props };
  ...
  useEffect(() => {
    ...
    function drawDisplay(target: FBO | null) {
      ...
      if (displayMaterial.uniforms.uBackColor) {
        gl.uniform3f(displayMaterial.uniforms.uBackColor, config.BACK_COLOR.r, config.BACK_COLOR.g, config.BACK_COLOR.b);
      }
      if (displayMaterial.uniforms.uTransparent) {
        gl.uniform1i(displayMaterial.uniforms.uTransparent, config.TRANSPARENT ? 1 : 0);
      }
      blit(target, false);
    }
  ...
  }, [finalConfig]); // Re-run effect if config changes

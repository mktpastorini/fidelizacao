...
export default function MenuPublicoPage() {
  ...
  const handleInitiateOrder = (produto: Produto, quantidade: number, observacoes: string) => {
    ...
    // Se a mesa estiver livre OU se o cliente principal não estiver definido, inicia o fluxo de identificação/cadastro
    if (!mesaData.cliente_id) {
        setIsIdentificationOpen(true);
    } else {
        // Se a mesa já estiver ocupada, usa o cliente principal (ou null para mesa geral)
  ...
  const handleOrderConfirmed = async (clienteId: string | null) => {
    ...
    const mesaEstavaLivre = !mesaData.cliente_id;

    try {
      // --- Lógica de Ocupação Automática (Se a mesa estava livre e um cliente foi identificado) ---
      if (mesaEstavaLivre && finalClienteId) {
        ...
        {!isMesaOcupada && (
            <Card className="mb-4 bg-yellow-900/20 text-yellow-100 border-yellow-700">
                <CardContent className="p-3 text-sm flex items-center gap-2">
                    ...
                    Esta mesa está livre. Clique em um produto ou em "Ocupar Mesa" para se identificar e começar a pedir.
                </CardContent>
            </Card>
        )}
